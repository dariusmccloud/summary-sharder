...
export function createNodeSqliteAdapter(dbPath) {
    ...
    return {
        ...
        run(sql, parameters = []) {
            return db.prepare(sql).run(...bindArgs(parameters));
        },
        get(sql, parameters = []) {
            return db.prepare(sql).get(...bindArgs(parameters));
        },
        all(sql, parameters = []) {
            return db.prepare(sql).all(...bindArgs(parameters));
        },
        scalar(sql, parameters = []) {
            return rowValue(db.prepare(sql).get(...bindArgs(parameters)));
        },
